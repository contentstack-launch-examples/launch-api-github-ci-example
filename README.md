# Deploy to Contentstack Launch with GitHub Actions using Launch API

This repository shows how to deploy a Next.js app to **Contentstack Launch** using the [Launch Public API](https://www.contentstack.com/docs/developers/apis/launch-api) file upload with **GitHub Actions**. The workflow (`.github/workflows/deploy.yml`) runs on push to `main` and uses the deployment script `deploy-api.js`.
...
3. Set the required variables as **GitHub Actions secrets**, or use `.env` for local runs.
